...
class GetAddressViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        let request = GetAddressRequest(token_type: (tokenResult?.tokenType)!, authorization: (tokenResult?.accessToken)!, type: addressType!, id: addressId!)
        ...
        Alamofire.request(request.getURL(),method: request.method, parameters: request.getParameters(), encoding: JSONEncoding.default, headers: request.getHeaders()).responseGetAddressResult { response in
            self.indicator.stopAnimating()
            ...
            switch response.result {
                case .success(let json):
                    ...
                    self.head.setTitle(result?.data.nickName, for: .normal)
                    self.name.text = result?.data.name
                    self.surname.text = result?.data.surname
                    self.provinceId = result?.data.cityID
                    self.province.setTitle(result?.data.cityName, for: .normal)
                    self.districtId = result?.data.townID
                    self.district.setTitle(result?.data.townName, for: .normal)
                    self.areaId = result?.data.areaID
                    self.area.setTitle(result?.data.areaName, for: .normal)
                    self.address.text = result?.data.addressText
            ...
            Alamofire.request(request.getURL(),method: request.method, parameters: request.getParameters(), encoding: JSONEncoding.default, headers: request.getHeaders()).responseGetContactResult { response in
                ...
                switch response.result {
                    case .success(let json):
                        ...
                        let request = SaveAddressRequest(token_type: (tokenResult?.tokenType)!, authorization: (tokenResult?.accessToken)!, type: self.addressType!, contactId: (response.result.value?.data.segments[0].contactID)!, nickName: self.head.currentTitle!, name: self.name.text!, surname: self.surname.text!, countryId: 90, cityId: self.provinceId!, cityName: self.province.currentTitle!, townId: self.districtId!, townName: self.district.currentTitle!, areaId: areaIdString, apartmentNo: "", nearestArea: "", addressText: self.address.text!, phone: self.phone.text!, taxNo: "", taxOffice: "", id: self.addressId!, identityNumber: "")
                        self.indicator.startAnimating()
                        /*print(request.getParameters())
                        Alamofire.request(request.getURL(), method: request.method, parameters: request.getParameters(), headers: request.getHeaders()).responseJSON { response in
                         self.indicator.stopAnimating()
                        ...
                        Alamofire.request(request.getURL(),method: request.method, parameters: request.getParameters(), encoding: JSONEncoding.default, headers: request.getHeaders()).responseSaveAddressResult { response in
                            ...
                            case .success(let json):
                                ...
                                if (result?.success)! {
                                    let alert = UIAlertController(title: "Tebrikler!", message: "Adres güncellendi!", preferredStyle: UIAlertControllerStyle.alert)
                                    ...
                                    let alert = UIAlertController(title: "Hata!", message: "Adres güncellenmedi!", preferredStyle: UIAlertControllerStyle.alert)
